...
class KNearestNeighbor(object):
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                #####################################################################
                # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
                dists[i][j] = 1 - (np.dot(X[i], self.X_train[j]) / (np.sqrt(np.sum(np.square(X[i]))) * np.sqrt(np.sum(np.square(self.X_train[j])))))
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            dists[i,:] = 1 - (np.dot(X[i], self.X_train.T) / (np.sqrt(np.sum(np.square(X[i]))) * np.sqrt(np.sum(np.square(self.X_train), axis=1))))
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        dists = 1 - (np.dot(X, self.X_train.T) / (np.sqrt(np.sum(np.square(X), axis=1))[:, np.newaxis] * np.sqrt(np.sum(np.square(self.X_train), axis=1))))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
